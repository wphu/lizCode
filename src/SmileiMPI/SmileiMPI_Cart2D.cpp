...
void SmileiMPI_Cart2D::exchangeParticles(Species* species, int ispec, PicParams& params, int tnum, int iDim)
{
    ...
    for (int iNeighbor=0 ; iNeighbor<nbNeighbors_ ; iNeighbor++)
    {
        ...
        if (neighbor_[iDim][(iNeighbor+1)%2]!=MPI_PROC_NULL) {
            ...
            MPI_Recv( &(buff_index_recv_sz[(iNeighbor+1)%2]), 1, MPI_INT, neighbor_[iDim][(iNeighbor+1)%2], 0, SMILEI_COMM_2D, &(rstat[(iNeighbor+1)%2]) );
        }
        else
            buff_index_recv_sz[(iNeighbor+1)%2] = 0;
    ...
    for (int iNeighbor=0 ; iNeighbor<nbNeighbors_ ; iNeighbor++)
    {
        ...
        if ( (neighbor_[iDim][(iNeighbor+1)%2]!=MPI_PROC_NULL) && (n_part_recv!=0) ) {
            ...
            MPI_Recv( &((partVectorRecv[(iNeighbor+1)%2]).position(0,0)), 1, typePartRecv,  neighbor_[iDim][(iNeighbor+1)%2], 0, SMILEI_COMM_2D, &(rstat[(iNeighbor+1)%2]) );
...
void SmileiMPI_Cart2D::gatherField0( Field* field_global ,Field* field  )
{
...
void SmileiMPI_Cart2D::gatherField1( Field* field_global ,Field* field  )
{
    int procs_rk;
    int iGlobal, jGlobal;
    int iGlobal_gather;
...
void SmileiMPI_Cart2D::gatherField1_all(Field* field_global ,Field* field)
{
    int procs_rk;
    int iGlobal, jGlobal;
    int iGlobal_gather;
    ...
    ny = f2D_global->dims_[1];
    f2D_global->put_to(0.0);
    MPI_Allgatherv(f2D->data_, send_cnt[smilei_rk], MPI_DOUBLE, field_global_gather, &recv_cnt[0], &recv_disp[0], MPI_DOUBLE, SMILEI_COMM_2D);
