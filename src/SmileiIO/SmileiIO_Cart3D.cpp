...
void SmileiIO_Cart3D::createDiagsPattern(PicParams& params, Diagnostic3D* diag3D)
{
    string diag_name;
    const char* h5_name;
    hid_t dataset_id;
...
void SmileiIO_Cart3D::write( PicParams& params, SmileiMPI* smpi, ElectroMagn* fields, vector<Species*>& vecSpecies, Diagnostic* diag, int itime)
{
    ...
    if( itime % params.dump_step == 0 && smpi->isMaster() )
    {
        ...
        // =============write Diagnostics ============================================
        createDiagsPattern(params, diag3D);

        diagsGroup.group_id = H5Gcreate(data_file_id, "/Diagnostic", H5P_DEFAULT, H5P_DEFAULT,H5P_DEFAULT);
        // particle flux
        iDiag = 0;
        diagsGroup.dims_global[0] = 1;
        diagsGroup.dims_global[1] = diag3D->n_species;
        diagsGroup.dims_global[2] = diag3D->n_segment_total;
        ...
        diagsGroup.dataset_id[iDiag] = H5Dcreate2(diagsGroup.group_id, h5_name, H5T_NATIVE_DOUBLE, diagsGroup.dataspace_id,
                                                    H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        for(int iSpec = 0; iSpec < diag3D->n_species; iSpec++)
        {
            int offset_line = 0;
            for(int iLine = 0; iLine < diag3D->n_line; iLine++)
            {
                ...
                diagsGroup.count[2] = diag3D->n_segments[iLine];
                ...
                diagsGroup.status = H5Dwrite(diagsGroup.dataset_id[iDiag], H5T_NATIVE_DOUBLE, diagsGroup.memspace_id,
                                        diagsGroup.dataspace_id, H5P_DEFAULT, &(diag3D->particleFlux_global[iSpec][iLine][0]));
                diagsGroup.status = H5Sclose(diagsGroup.memspace_id);
                offset_line += diag3D->n_segments[iLine];
            }
        }
        ...
        for(int iSpec = 0; iSpec < diag3D->n_species; iSpec++)
        {
            ...
            for(int iLine = 0; iLine < diag3D->n_line; iLine++)
            {
                ...
                diagsGroup.status = H5Sselect_hyperslab(diagsGroup.dataspace_id, H5S_SELECT_SET, diagsGroup.offset,
                                                    diagsGroup.stride, diagsGroup.count, diagsGroup.block);
                diagsGroup.status = H5Dwrite(diagsGroup.dataset_id[iDiag], H5T_NATIVE_DOUBLE, diagsGroup.memspace_id,
                                        diagsGroup.dataspace_id, H5P_DEFAULT, &(diag3D->heatFlux_global[iSpec][iLine][0]));
                diagsGroup.status = H5Sclose(diagsGroup.memspace_id);
                offset_line += diag3D->n_segments[iLine];
            }
        }
        ...
        for(int iSpec = 0; iSpec < diag3D->n_species; iSpec++)
        {
            ...
            for(int iLine = 0; iLine < diag3D->n_line; iLine++)
            {
                ...
                diagsGroup.status = H5Sselect_hyperslab(diagsGroup.dataspace_id, H5S_SELECT_SET, diagsGroup.offset,
                                                    diagsGroup.stride, diagsGroup.count, diagsGroup.block);
                diagsGroup.status = H5Dwrite(diagsGroup.dataset_id[iDiag], H5T_NATIVE_DOUBLE, diagsGroup.memspace_id,
                                        diagsGroup.dataspace_id, H5P_DEFAULT, &(diag3D->averageAngle_global[iSpec][iLine][0]));
                diagsGroup.status = H5Sclose(diagsGroup.memspace_id);
                offset_line += diag3D->n_segments[iLine];
            }
        }
        ...
        diagsGroup.dataset_id[iDiag] = H5Dcreate2(diagsGroup.group_id, h5_name, H5T_NATIVE_DOUBLE, diagsGroup.dataspace_id,
                                                    H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
        for(int iPsi = 0; iPsi < diag3D->psiRate_global.size(); iPsi++)
        {
            ...
            diagsGroup.status = H5Sselect_hyperslab(diagsGroup.dataspace_id, H5S_SELECT_SET, diagsGroup.offset,
                                                diagsGroup.stride, diagsGroup.count, diagsGroup.block);
            diagsGroup.status = H5Dwrite(diagsGroup.dataset_id[iDiag], H5T_NATIVE_DOUBLE, diagsGroup.memspace_id,
                                    diagsGroup.dataspace_id, H5P_DEFAULT, (diag3D->psiRate_global[iPsi])->data_);
            diagsGroup.status = H5Sclose(diagsGroup.memspace_id);
        }
        diagsGroup.status = H5Sclose(diagsGroup.dataspace_id);
        diagsGroup.status = H5Dclose(diagsGroup.dataset_id[iDiag]);
...
void SmileiIO_Cart3D::writeGrid(Grid* grid)
{
    ...
    grid_status = H5Dwrite(grid_dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, grid3D->iswall_global_);
    ...
    grid_dataset_id = H5Dcreate2(grid_file_id, grid_dataset_name.c_str(), H5T_NATIVE_INT, grid_dataspace_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
    grid_status = H5Dwrite(grid_dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, grid3D->bndr_global_);
    grid_status = H5Sclose(grid_dataspace_id);
    grid_status = H5Dclose(grid_dataset_id);
    ...
    grid_status = H5Dwrite(grid_dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, grid3D->bndrVal_global_);
    grid_status = H5Sclose(grid_dataspace_id);
    grid_status = H5Dclose(grid_dataset_id);

    /*
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0; iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            cout<<grid3D->lines[iLine][iSegment].grid_point[0]<<" "<<grid3D->lines[iLine][iSegment].grid_point[1]<<endl;
            cout<<grid3D->lines[iLine][iSegment].start_point[0]<<" "<<grid3D->lines[iLine][iSegment].start_point[1]<<endl;
            cout<<grid3D->lines[iLine][iSegment].end_point[0]<<" "<<grid3D->lines[iLine][iSegment].end_point[1]<<endl;
        }
        cout<<endl;
        cout<<"================================"<<endl;
    }
    */
    ...
    int *n_segments = new int[grid3D->n_line];
    for(int i = 0; i < grid3D->n_line; i++)
    {
        n_segments[i] = grid3D->n_segments[i];
    }

    double *start_point = new double[grid3D->n_segment_total * 2];
    ii = 0;
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            start_point[ii * 2]     = grid3D->lines[iLine][iSegment].start_point[0];
            start_point[ii * 2 + 1] = grid3D->lines[iLine][iSegment].start_point[1];
            ii++;
        }
    }

    double *end_point = new double[grid3D->n_segment_total * 2];
    ii = 0;
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            end_point[ii * 2]     = grid3D->lines[iLine][iSegment].end_point[0];
            end_point[ii * 2 + 1] = grid3D->lines[iLine][iSegment].end_point[1];
            ii++;
        }
    }

    double *length = new double[grid3D->n_segment_total];
    ii = 0;
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            length[ii] = grid3D->lines[iLine][iSegment].length;
            ii++;
        }
    }

    int *grid_point0 = new int[grid3D->n_segment_total * 2];
    ii = 0;
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid_point0[ii * 2]     = grid3D->lines[iLine][iSegment].grid_point0[0];
            grid_point0[ii * 2 + 1] = grid3D->lines[iLine][iSegment].grid_point0[1];
            ii++;
        }
    }

    int *grid_point1 = new int[grid3D->n_segment_total * 2];
    ii = 0;
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid_point1[ii * 2]     = grid3D->lines[iLine][iSegment].grid_point1[0];
            grid_point1[ii * 2 + 1] = grid3D->lines[iLine][iSegment].grid_point1[1];
            ii++;
        }
    }

    double *normal = new double[grid3D->n_segment_total * 3];
    ii = 0;
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            normal[ii * 3]     = grid3D->lines[iLine][iSegment].normal[0];
            normal[ii * 3 + 1] = grid3D->lines[iLine][iSegment].normal[1];
            normal[ii * 3 + 2] = grid3D->lines[iLine][iSegment].normal[2];
            ii++;
        }
    }
    ...
    // write start_point
    ndim_segment = 2;
    dim2d_segment[0] = grid3D->n_segment_total;
    dim2d_segment[1] = 2;
    ...
    // write length
    ndim_segment = 1;
    dim1d_segment[0] = grid3D->n_segment_total;
    segment_dataspace_id = H5Screate_simple(ndim_segment, dim1d_segment, NULL);
...
void SmileiIO_Cart3D::readGrid(Grid* grid)
{
    ...
    grid_status = H5Dread(grid_dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, grid3D->iswall_global_);
    grid_status = H5Dclose(grid_dataset_id);

    grid_dataset_name = "bndr_type";
    grid_dataset_id = H5Dopen2(grid_file_id, grid_dataset_name.c_str(), H5P_DEFAULT);    
    grid_status = H5Dread(grid_dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, grid3D->bndr_global_);
    grid_status = H5Dclose(grid_dataset_id);

    grid_dataset_name = "bndr_val";
    grid_dataset_id = H5Dopen2(grid_file_id, grid_dataset_name.c_str(), H5P_DEFAULT);        
    grid_status = H5Dread(grid_dataset_id, H5T_NATIVE_DOUBLE, H5S_ALL, H5S_ALL, H5P_DEFAULT, grid3D->bndrVal_global_);
    grid_status = H5Dclose(grid_dataset_id);
    ...
    */
    ...
    grid3D->n_line = dim1d_segment[0];

    int *n_segments = new int[grid3D->n_line];
    H5Dread(segment_dataset_id, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, H5P_DEFAULT, n_segments);
    grid3D->n_segments.resize(grid3D->n_line);
    grid3D->n_segment_total = 0;
    for(int i = 0; i < grid3D->n_line; i++)
    {
        grid3D->n_segments[i] = n_segments[i];
        grid3D->n_segment_total += n_segments[i];
    }
    H5Sclose(segment_dataspace_id);
    H5Dclose(segment_dataset_id);

    grid3D->lines.resize(grid3D->n_line);
    for(int iLine = 0; iLine < grid3D->n_line; iLine++)
    {
        grid3D->lines[iLine].resize(grid3D->n_segments[iLine]);
    }
    ...
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid3D->lines[iLine][iSegment].start_point[0] = start_point[ii * 2];
            grid3D->lines[iLine][iSegment].start_point[1] = start_point[ii * 2 + 1];
            ii++;
        }
    }
    ...
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid3D->lines[iLine][iSegment].end_point[0] = end_point[ii * 2];
            grid3D->lines[iLine][iSegment].end_point[1] = end_point[ii * 2 + 1];
            ii++;
        }
    }
    ...
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid3D->lines[iLine][iSegment].length = length[ii];
            ii++;
        }
    }
    ...
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid3D->lines[iLine][iSegment].grid_point0[0] = grid_point0[ii * 2];
            grid3D->lines[iLine][iSegment].grid_point0[1] = grid_point0[ii * 2 + 1];
            ii++;
        }
    }
    ...
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid3D->lines[iLine][iSegment].grid_point1[0] = grid_point1[ii * 2];
            grid3D->lines[iLine][iSegment].grid_point1[1] = grid_point1[ii * 2 + 1];
            ii++;
        }
    }
    ...
    for(int iLine = 0; iLine < grid3D->lines.size(); iLine++)
    {
        for(int iSegment = 0;  iSegment < grid3D->lines[iLine].size(); iSegment++)
        {
            grid3D->lines[iLine][iSegment].normal[0] = normal[ii * 3];
            grid3D->lines[iLine][iSegment].normal[1] = normal[ii * 3 + 1];
            grid3D->lines[iLine][iSegment].normal[2] = normal[ii * 3 + 2];
            ii++;
        }
    }
    grid_status = H5Fclose(grid_file_id);
